...
function C2 = admmLasso_mat_func_CSI_SSC(Y,affine,alpha,ks,thr,maxIter,lambda,alphass,Md,Nd)
...
if (~affine)
    ...
    Q = gpuArray(Q);
    C1 = zeros(N,N,'single'); % C^k
    %C1 = gpuArray(C1);
    C_average = zeros(N,N,'single'); %\bar{C}k^k
    %C_average = gpuArray(C_average);
    Lambda2 = zeros(N,N,'single'); % Delta^k
    %Lambda2 = gpuArray(Lambda2);
    lambda3 = zeros(1,N,'single'); % delta^kT
    ...
    while ( (err1(i) > thr1 || err3(i) > thr1) && i < maxIter )
        ...
        C_averagetemp = imgaussfilt3(C_temp,ks,'padding','symmetric');
